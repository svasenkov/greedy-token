...
Под капотом: каждая задача уходит в самый дешёвый способный tier — `rg`/`jq` tools, trust-одобренные workspace-скрипты, локальная Ollama или lexical RAG — и эскалирует в agent chat только когда дешевле некуда. Workspace-скрипты запускаются только после явного `trust add`, SHA-256 и file identity перепроверяются перед каждым запуском. Повторяющиеся паттерны телеметрии кристаллизуются в ревьюабельные draft-роуты через аудируемый lifecycle `candidate → proposed → approved → applied` — ничего не активируется без человеческого `promote`. `greedy_token_capabilities` показывает derived-инвентарь invocable операций; телеметрия пишется в `~/.greedy-token/usage.jsonl`.

<details>
<summary><strong>Отзыв</strong> (письмо модели — по желанию)</summary>
